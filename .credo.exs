    ...
    %{
      ...
      checks: %{
        enabled: [
          ...
          # JSON-RPC 2.0 error codes (-32700, -32600, -32601, …, -32001) are an
          # external protocol constant set; the spec and the whole ecosystem write
          # them without underscores. Only flag genuinely large quantities (6+
          # digits), where underscores actually aid readability.
          ...
          #
          ## ExSlop — AI-generated code patterns (recommended set)
          #
          # The `plugins:` entry above can't enable these on its own
          # because our explicit `enabled:` list replaces (not merges)
          # the plugin's `extra:` injection. Cherry-picking here gives
          # us the same coverage with visible intent.
          #
